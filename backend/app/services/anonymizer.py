...
def _build_llm_prompt(text: str) -> str:
    return (
        ...
        "(ФИО, адреса, телефоны, e-mail, @username, ИНН, СНИЛС, ОГРН, ОГРНИП, КПП, БИК, номера счетов, паспорта, даты рождения, реквизиты). "
        "Любые адреса (населенные пункты, улицы, дома, квартиры) помечай как [АДРЕС]. "
        "Любые e-mail и строки с символом '@' (например Telegram/соц.сети) помечай как [EMAIL]. "
        "Если видишь последовательности из 6+ цифр, похожие на реквизиты/идентификаторы, включай их. "
