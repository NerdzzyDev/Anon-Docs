...
class NameDetector:
    ...
    def _regex_spans(self, text: str) -> List[DetectedSpan]:
        patterns: list[tuple[re.Pattern[str], int]] = [
            # Наименование / ИП + ФИО (берем только ФИО)
            (re.compile(r"\bНаименование\s*:\s*(?:ИП|ИНДИВИДУАЛЬНЫЙ\s+ПРЕДПРИНИМАТЕЛЬ)\s+([А-ЯЁ]{2,}(?:\s+[А-ЯЁ]{2,}){1,2})\b", re.IGNORECASE), 1),
            (re.compile(r"\b(?:ИП|ИНДИВИДУАЛЬНЫЙ\s+ПРЕДПРИНИМАТЕЛЬ)\s+([А-ЯЁ]{2,}(?:\s+[А-ЯЁ]{2,}){1,2})\b", re.IGNORECASE), 1),
